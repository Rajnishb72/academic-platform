...
import {
  ...
  Flame, ChevronRight, BarChart2, Zap,
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // ── Personalized recommendations: notes from user's subjects
      const userSubjects = [...new Set(uploads.map((n: { subject?: string }) => n.subject).filter(Boolean))];
      if (userSubjects.length > 0) {
        const { data: recData } = await supabase
          .from("notes")
          .select("id,title,subject,avg_rating,views_count")
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 space-y-7">
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid var(--ax-border)", background: "var(--ax-surface-2)" }}>
          ...
          <div className="flex items-center gap-4 px-6 py-5">
            ...
            <div className="hidden sm:flex gap-4">
              <div className="h-14 w-28 ax-skeleton rounded-2xl" />
              <div className="h-14 w-28 ax-skeleton rounded-2xl" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-5">
            {[...Array(2)].map((_, i) => (
              <div key={i} className="rounded-2xl p-5 space-y-4" style={{ border: "1px solid var(--ax-border)", background: "var(--ax-surface-2)" }}>
                <div className="flex items-center gap-2">
                  <div className="h-4 w-4 ax-skeleton rounded" />
                  <div className="h-3 w-32 ax-skeleton rounded" />
                </div>
                {[...Array(3)].map((_, j) => (
                  <div key={j} className="flex items-center gap-3">
                    <div className="h-8 w-8 ax-skeleton rounded-lg shrink-0" />
                    <div className="flex-1 space-y-1.5">
                      <div className="h-2.5 w-3/4 ax-skeleton rounded" />
                      <div className="h-2 w-1/2 ax-skeleton rounded" />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-5">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="rounded-2xl p-5 space-y-4" style={{ border: "1px solid var(--ax-border)", background: "var(--ax-surface-2)" }}>
                <div className="h-3 w-28 ax-skeleton rounded" />
                {[...Array(3)].map((_, j) => (
                  <div key={j} className="h-10 w-full ax-skeleton rounded-xl" />
                ))}
              </div>
            ))}
          </div>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 ax-ambient-glow min-h-screen">
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between gap-4 px-6 py-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs" style={{ color: "var(--ax-text-muted)" }}>{greeting()},</p>
      ...
      <OnboardingChecklist stats={{
        ...
        aiUsed: typeof window !== "undefined" && localStorage.getItem("academix_ai_used") === "1",
      }} />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-6 lg:col-span-2">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {/* My Groups */}
            <Panel title="My Campus Groups" icon={School} href="/campus/my-campus" color="text-emerald-400" delay={0.35}>
              {groups.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {groups.slice(0, 4).map((g, i) => (
                    <motion.div key={g.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 + i * 0.05 }}>
            ...
            {/* Forum Posts */}
            <Panel title="My Forum Posts" icon={MessageSquare} href="/forums" color="text-indigo-400" delay={0.38}>
              {recentPosts.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {recentPosts.map((p, i) => (
                    <motion.div key={p.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.38 + i * 0.04 }}
          ...
          {/* Library */}
          <Panel title="Library" icon={BookOpen} href="/library" color="text-amber-400" delay={0.42}>
        ...
        <div className="flex flex-col gap-6">
          ...
          {/* Study Plans */}
          <Panel title="Study Plans" icon={Target} href="/planner" color="text-violet-400" delay={0.33}>
            {studyPlans.length === 0 ? (
              <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <div className="flex flex-col gap-3">
                {studyPlans.map((p, i) => {
                  ...
                  return (
                    <motion.div key={p.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.33 + i * 0.05 }}>
          ...
          {/* Quick Actions */}
          <Panel title="Quick Actions" icon={Sparkles} color="text-amber-400" delay={0.45}>
            <div className="flex flex-col gap-2">
              {[
                ...
                <motion.div key={href} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.45 + i * 0.04 }}>
          ...
          {/* Campus Summary */}
          <Panel title="Campus Summary" icon={School} href="/campus/my-campus" color="text-emerald-400" delay={0.52}>
          ...
          {/* ── Trending in Library ─────────────────────────────── */}
          <Panel title="Trending in Library" icon={TrendingUp} href="/library" color="text-amber-400" delay={0.55}>
            {trendingNotes.length === 0 ? (
              <div className="flex flex-col items-center gap-3 py-6 text-center">
                ...
                <Link href="/library" className="text-xs text-amber-400 hover:underline">Browse Library →</Link>
              </div>
            ) : (
              <div className="flex flex-col gap-2">
                {trendingNotes.map((n, i) => (
                  <motion.div key={n.id} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.55 + i * 0.04 }}>
                    <Link href="/library" className="flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors hover:bg-[var(--ax-surface-hover)] group"
                      style={{ border: "1px solid var(--ax-border)" }}>
                      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-amber-500/10 text-[10px] font-bold text-amber-400 ring-1 ring-amber-500/20">
                        {i + 1}
                      </div>
                      <div className="min-w-0 flex-1">
                      ...
                      <div className="flex flex-col items-end gap-0.5">
                        <span className="flex items-center gap-0.5 text-[10px] text-amber-400">
                          <Eye className="h-2.5 w-2.5" />{n.views_count}
                        </span>
          ...
          {/* ── Hot Discussions ─────────────────────────────────── */}
          <Panel title="Hot Discussions" icon={Flame} href="/forums" color="text-rose-400" delay={0.60}>
            {hotPosts.length === 0 ? (
              <div className="flex flex-col items-center gap-3 py-6 text-center">
                <MessageSquare className="h-8 w-8" style={{ color: "var(--ax-text-faint)" }} />
                <p className="text-sm" style={{ color: "var(--ax-text-muted)" }}>No hot discussions yet</p>
                <Link href="/forums/create" className="text-xs text-rose-400 hover:underline">Start one →</Link>
              </div>
            ) : (
              <div className="flex flex-col gap-2">
                {hotPosts.map((p, i) => (
                  <motion.div key={p.id} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.60 + i * 0.04 }}
                    className="rounded-xl px-3 py-2.5 transition-colors hover:bg-[var(--ax-surface-hover)]"
                    style={{ border: "1px solid var(--ax-border)" }}>
                    <p className="truncate text-xs font-medium" style={{ color: "var(--ax-text-primary)" }}>{p.title}</p>
                    <div className="mt-1 flex items-center justify-between text-[10px]" style={{ color: "var(--ax-text-faint)" }}>
          ...
          {/* ── Recommended For You ─────────────────────────────── */}
          <Panel title="Recommended For You" icon={Zap} href="/library" color="text-violet-400" delay={0.65}>
            {recommended.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {recommended.map((n, i) => (
                  <motion.div key={n.id} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.65 + i * 0.04 }}>
                    <Link href="/library" className="flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors hover:bg-[var(--ax-surface-hover)] group"
                      style={{ border: "1px solid var(--ax-border)" }}>
