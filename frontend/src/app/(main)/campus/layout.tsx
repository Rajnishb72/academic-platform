...
import { Building2, School, KeyRound, Settings2 } from "lucide-react";

const NAV = [
  { label: "Groups", href: "/campus", icon: Building2 },
  { label: "My Groups", href: "/campus/my-campus", icon: School },
  { label: "Join with Code", href: "/campus/join", icon: KeyRound },
  { label: "Edit Group", href: "/campus/manage", icon: Settings2 },
] as const;
  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)] flex-col" style={{ background: "var(--ax-surface-0)" }}>
      ...
      <div className="relative overflow-hidden border-b" style={{ borderColor: "var(--ax-border)" }}>
        <div className="absolute inset-0 bg-gradient-to-r from-emerald-600/10 via-teal-600/5 to-transparent" />
        ...
        <div className="relative mx-auto flex max-w-7xl items-center gap-4 px-6 py-5">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "var(--ax-text-muted)" }}>
              Groups · discussions, assignments & community
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="sticky top-16 z-20 border-b" style={{ borderColor: "var(--ax-border)", background: "rgba(5, 8, 22, 0.85)", backdropFilter: "blur(16px)" }}>
        <div className="mx-auto max-w-7xl px-6">
          <nav className="flex gap-1 overflow-x-auto py-2 scrollbar-hide" aria-label="Campus navigation">
      ...
      {/* ── Page Content ── */}
      <div className="flex-1" style={{ background: "var(--ax-surface-0)" }}>
