...
import {
  ...
  getLibraryRank,
  getNextLibraryRank,
} from "@/lib/library-ranks";
import { computeUserXP, XPBreakdown } from "@/lib/xp";
import {
  followUser,
  unfollowUser,
  getFollowers,
...
function getBadges(d: ProfileData, totalUpvotes: number) {
  ...
  const xpEst = d.xpData?.totalXP ?? 0;
...
export default function ProfilePage() {
  ...
  const [previewNote, setPreviewNote] = useState<
    ProfileData["libraryNotes"][0] | null
  >(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Run three independent data tasks in parallel
      const assignmentsTask = (async () => {
        let submitted = 0;
        await Promise.all(
      ...
      const libraryTask = (async () => {
        ...
        if (libraryResult.data && libraryResult.data.length > 0) {
          ...
          libStats = computeLibraryStats(libraryResult.data, viewCount, bookmarkCount);
          libraryXP = calcLibXP(libStats);
        }
        return { libraryXP, libStats };
      })();

      const plannerTask = (async (): Promise<{ plannerSubmittedCount: number; plannerMilestoneXP: number; plannerTitles: { id: string; title: string; source: string }[]; planCount: number; planChapters: number }> => {
        ...
        try {
          if (plansResult.data) {
            ...
            planChapters = plans.reduce(
              (s, p) => s + (p.plan.schedule?.length || 0),
              0,
            );
            plannerSubmittedCount = plans.reduce(
              (s, p) => s + (p.proofs?.length || 0),
              0,
            );

            const states = computeMilestoneStates(plans);
            const achievedMilestones = states
              .filter((s) => s.achieved)
              .map((s) => ALL_MILESTONES.find((x) => x.id === s.id))
              .filter(Boolean) as typeof ALL_MILESTONES;

            plannerMilestoneXP = achievedMilestones.reduce(
              (sum, m) => sum + m.xp,
              0,
            );
            plannerTitles = achievedMilestones.map((m) => ({
              id: m.id,
              title: m.title,
              source: "Planner Milestone",
            }));
          }
        } catch (e) {
          console.error("Planner stats error", e);
        }
        return { plannerSubmittedCount, plannerMilestoneXP, plannerTitles, planCount, planChapters };
      })();

      const xpTask = computeUserXP(user.id).catch(() => null as XPBreakdown | null);

      const [submitted, libResult, plannerResult, xpData] = await Promise.all([
        assignmentsTask, libraryTask, plannerTask, xpTask,
      ]);

      const { libraryXP, libStats } = libResult;
      const { plannerSubmittedCount, plannerMilestoneXP, plannerTitles, planCount, planChapters } = plannerResult;
  ...
  const rep = xpData?.forum.upvotesReceived ?? 0;
  const title = xpData?.title.name ?? "Newcomer";
  const titleColor = xpData?.title.color ?? "text-zinc-500";
  const level = xpData?.title.rank ?? 1;
  const xpCurrent = xpData ? xpData.totalXP - xpData.title.threshold : 0;
  const xpNext = xpData?.nextTitle
    ? xpData.nextTitle.threshold - xpData.title.threshold
    : 100;
  const xpPct = xpData?.progressPct ?? 0;
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-8">
      ...
      <AnimatePresence mode="wait">
          ...
          >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    <button
                      ...
                      className={`rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider transition hover:scale-105 ${titleColor} bg-slate-800 border-slate-700`}
                    >
                      {title}
                    </button>
                  ...
                  {isCustomTitle && (
                    <div className="mt-1.5">
                      <span
                        className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-0.5 rounded-full border border-purple-500/30 bg-purple-500/10 text-purple-400`}
                        title="Equipped Title — change from Planner › Milestones"
                      >
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              {(() => {
                ...
                const rkPct = nxtRank
                  ? Math.min(
                    ((plannerXP - curRank.threshold) /
                      (nxtRank.threshold - curRank.threshold)) *
                    100,
                    100,
                  )
                  : 100;
                ...
                return (
                  <div className="rounded-2xl border border-slate-800/60 bg-slate-900/50 p-4 shadow-[0_0_15px_rgba(0,0,0,0.2)]">
                    <div className="flex items-center gap-3 mb-3">
                      <div
                        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${curRank.bgColor} ${curRank.borderColor} ${curRank.glow}`}
                      >
                        <RankIcon
                          name={curRank.icon}
                          cls={`h-4.5 w-4.5 ${curRank.color}`}
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-sm font-bold ${curRank.color}`}>
                          {curRank.name}
                        </p>
                      </div>
                    </div>
              ...
              {(() => {
                ...
                return (
                  <div className="rounded-2xl border border-slate-800/60 bg-slate-900/50 p-4 shadow-[0_0_15px_rgba(0,0,0,0.2)]">
                    <div className="flex items-center gap-3 mb-3">
                      <div
                        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-950/50 border ${curRank.borderX} ${curRank.glow}`}
                      >
              ...
              {/* Forum Tier Card */}
              {(() => {
                const forumRep = profileData
                  ? profileData.forumPosts.reduce(
                    (s, p) =>
                      s +
                      (p.upvotes_count ?? 0) +
                      10 +
                ...
                return (
                  <div className="rounded-2xl border border-slate-800/60 bg-slate-900/50 p-4 shadow-[0_0_15px_rgba(0,0,0,0.2)]">
                    <div className="flex items-center gap-3 mb-3">
                      <div
                        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-950/50 border ${forumTier.ring}`}
                      >
                        <MessageSquare
                          className={`h-4.5 w-4.5 ${forumTier.color}`}
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                        ...
                        <p className={`text-sm font-bold ${forumTier.color}`}>
                          {forumTier.label}
                        </p>
                      </div>
                    </div>
                      ...
                      <>
                        <div className="h-2 w-full overflow-hidden rounded-full bg-slate-800">
                          <motion.div
                            ...
                            initial={{ width: 0 }}
                            animate={{
                              width: `${Math.min(((forumRep - forumTier.min) / (forumTier.max - forumTier.min)) * 100, 100)}%`,
                            }}
                            transition={{
                              duration: 1,
                              ease: "easeOut",
            ...
            <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
              {[
                {
                  label: "Groups",
                  value: profileData?.groupCount ?? 0,
                  icon: School,
                  color: "text-emerald-400",
                },
                {
                  label: "Forum Posts",
                  value: profileData?.forumPosts.length ?? 0,
                  icon: MessageSquare,
                  color: "text-purple-400",
                },
                {
                  label: "Notes Uploaded",
                  value: profileData?.libraryNotes.length ?? 0,
                  icon: BookOpen,
                  color: "text-amber-400",
                },
                {
                  label: "Assignments",
                  value: profileData?.submittedCount ?? 0,
                  icon: CheckCircle2,
                  color: "text-teal-400",
                },
                {
                ...
                {
                  label: "Badges",
                  value: `${profileData ? getBadges(profileData, rep).filter((b) => b.earned).length : 0}/${profileData ? getBadges(profileData, rep).length : 0}`,
                  icon: Award,
                  color: "text-amber-400",
                },
                ...
                <div
                  key={label}
                  className="flex flex-col items-center gap-1 rounded-xl border border-slate-800/60 bg-slate-900/50 py-4"
                >
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-slate-500">
                          {g.member_count} members · {g.assignment_count ?? 0}{" "}
                          assignments
                        </p>
                      </div>
                      <span
                        className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold capitalize ring-1 ${g.userRole === "owner"
                ...
                <div className="flex flex-col gap-2">
                  {profileData?.libraryNotes.map((n, i) => (
                    <motion.button
                      key={n.id}
                      onClick={() =>
                        n.file_url ? setPreviewNote(n) : undefined
                      }
                      initial={{ opacity: 0, x: -6 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: i * 0.04 }}
          ...
          >
            <div className="rounded-2xl border border-slate-800/60 bg-slate-900/50 p-6 shadow-[0_0_15px_rgba(0,0,0,0.2)]">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    Complete study plans, submit proofs, and maintain streaks to
                    earn these.
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-black text-indigo-400">
                    {profileData
                      ? getBadges(profileData, rep).filter((b) => b.earned)
                        .length
                      : 0}
                  </p>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {profileData
                  ? getBadges(profileData, rep).map((b) => {
                    const BIcon = b.icon;
                    return (
                      <div
                        key={b.id}
                      ...
                      >
                        <div
                          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${b.earned ? "bg-indigo-500/20" : "bg-slate-700"}`}
                        >
                          {b.earned ? (
                        ...
                        <div className="min-w-0 flex-1">
                          <p
                            className={`truncate text-sm font-bold ${b.earned ? "text-indigo-300" : "text-slate-400"}`}
                          >
                            {b.label}
                          </p>
            ...
            <div className="rounded-2xl border border-slate-800/60 bg-slate-900/50 p-6 shadow-[0_0_15px_rgba(0,0,0,0.2)]">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    Upload notes, gain downloads, ratings, and bookmarks to
                    collect these.
                  </p>
                </div>
                ...
                {profileData &&
                  (() => {
                    const stats = {
                      ...
                      totalDownloads: profileData.libraryNotes.reduce(
                        (s, n) => s + (n.downloads_count ?? 0),
                        0,
                      ),
                      totalViews: 0, // In full app we might retrieve this, but we approximate for profile showcase
                      totalBookmarks: 0,
                      totalRatings: profileData.libraryNotes.reduce(
                        (s, n) =>
                          s + (n.avg_rating && n.avg_rating >= 4 ? 1 : 0),
                        0,
                      ),
                      topNoteDownloads: Math.max(
                        0,
                        ...profileData.libraryNotes.map(
                          (n) => n.downloads_count ?? 0,
                        ),
                      ),
                      notesAbove50Downloads: profileData.libraryNotes.filter(
                        (n) => (n.downloads_count ?? 0) >= 50,
                      ).length,
                      subjectsUploaded: new Set(
                        profileData.libraryNotes.map((n) => n.subject || ""),
                      ),
                      consecutiveWeeks: 0,
                    };
                    ...
                    return ALL_LIB_MILESTONES.map((m: any) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${achieved ? "bg-amber-500/20" : "bg-slate-700"}`}
                          >
                            {achieved ? (
                          ...
                          <div className="min-w-0 flex-1">
                            <p
                              className={`truncate text-sm font-bold ${achieved ? "text-amber-300" : "text-slate-400"}`}
                            >
                              {m.label}
                            </p>
                            <p className="truncate text-[10px] text-slate-500">
        ...
        {tab === "achievements" && (
          ...
          >
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {badges.map((b, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${b.earned ? "bg-current/10" : "bg-slate-800"}`}
                      >
            ...
            {(() => {
              ...
              > = {
                progress: {
                  label: "The Completionist Path",
                  color: "text-blue-400",
                },
                streak: {
                  label: "The Consistency Path",
                  color: "text-orange-400",
                },
                goals: {
                  label: "The Execution Path",
                  color: "text-emerald-400",
                },
                mastery: {
                  label: "The Speedster Path",
                  color: "text-purple-400",
                },
              };
              ...
              const PLANNER_TITLES = [
                {
                  id: "prog_legendary",
                  name: "Completionist Grandmaster",
                  rarity: "Mythic",
                  head: "bg-clip-text text-transparent bg-gradient-to-r from-rose-400 to-orange-400 font-black",
                },
                {
                  id: "prog_heroic",
                  name: "Academic Titan",
                  rarity: "Legendary",
                  head: "text-violet-400 font-extrabold",
                },
                {
                  id: "prog_diamond",
                  name: "Full Scholar",
                ...
                },
                {
                  ...
                  name: "Eternal Grind",
                  rarity: "Mythic",
                  head: "bg-clip-text text-transparent bg-gradient-to-r from-red-400 to-rose-400 font-black",
                },
                {
                  id: "streak_diamond",
                  name: "Unbreakable",
                ...
                },
                {
                  ...
                  rarity: "Rare",
                  head: "text-indigo-400 font-semibold",
                },
                {
                  id: "streak_gold",
                  name: "Relentless",
                ...
                {
                  id: "ontime_legendary",
                  name: "Temporal Grandmaster",
                  rarity: "Mythic",
                  head: "bg-clip-text text-transparent bg-gradient-to-r from-fuchsia-400 to-purple-400 font-black",
                },
                {
                  id: "ontime_heroic",
                  name: "Deadline Assassin",
                  rarity: "Legendary",
                  head: "text-violet-400 font-extrabold",
                },
                {
              ...
              try {
                const ms = localStorage.getItem("academix_milestones_v2");
                if (ms) milestoneStates = JSON.parse(ms);
              } catch {
                /* ignore */
              }
              const stateMap = new Map(milestoneStates.map((m) => [m.id, m]));
              const plannerEarned = ALL_MILESTONES.filter(
                (m) => stateMap.get(m.id)?.achieved,
              ).length;
              ...
              const currentPlannerTitle =
                PLANNER_TITLES.find(
                  (t) =>
                    t.name === selectedTitle && stateMap.get(t.id)?.achieved,
                ) ??
                ...
                <div className="rounded-2xl border border-slate-800/60 bg-slate-900/50 p-5 space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-500">
                      ...
                      / {ALL_MILESTONES.length} unlocked
                    </span>
                  </div>

                  {/* Dynamic Planner Title card */}
                  <div
                    className={`rounded-xl border p-4 flex items-center gap-4 ${currentPlannerTitle ? "border-indigo-500/30 bg-indigo-500/10 shadow-[0_0_15px_rgba(99,102,241,0.1)]" : "border-slate-800 bg-slate-800/50"}`}
                  >
                    <div
                      className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${currentPlannerTitle ? "bg-indigo-500/20" : "bg-slate-800"}`}
                    >
                      <Award
                        className={`h-5 w-5 ${currentPlannerTitle ? "text-indigo-400" : "text-slate-600"}`}
                      />
                    </div>
                    <div className="flex-1">
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                        Active Planner Title
                      </p>
                      {currentPlannerTitle ? (
                        <div className="flex items-center gap-2">
                          <p
                            className={`text-sm font-bold ${currentPlannerTitle.head}`}
                          >
                            {currentPlannerTitle.name}
                          </p>
                          {"rarity" in currentPlannerTitle && (
                            <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">
                              {(currentPlannerTitle as any).rarity}
                            </span>
                          )}
                        </div>
                      ) : (
                        <p className="text-sm text-slate-400">
                          ...
                          <span className="text-[10px]">
                            — submit your first proof to earn a title
                          </span>
                        </p>
                      )}
                    </div>
                  ...
                  {cats.map((cat) => {
                    ...
                    const catMs = ALL_MILESTONES.filter(
                      (m) => m.category === cat,
                    );
                    const catEarned = catMs.filter(
                      (m) => stateMap.get(m.id)?.achieved,
                    ).length;
                      ...
                      <div key={cat} className="space-y-3">
                        ...
                        <div className="grid gap-2 sm:grid-cols-2">
                          {catMs.map((m, i) => {
                            ...
                            const style =
                              TIER_STYLES[m.tier] ?? TIER_STYLES.bronze;
                            ...
                            return (
                              ...
                              >
                                {/* Icon */}
                                <div
                                  className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${earned ? `${style.ring} shadow-inner bg-slate-900/50` : "bg-slate-800/60 ring-1 ring-slate-700/50"}`}
                                >
                                  {renderIcon(
                                    m.icon,
                                    `h-4 w-4 ${earned ? style.icon : "text-slate-600"}`,
                                  )}
                                </div>
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <p
                                      className={`text-xs font-bold leading-tight ${earned ? "text-slate-200" : "text-slate-500"}`}
                                    >
                                      {m.title}
                                    </p>
                                    <span
                                      className={`text-[8px] font-black uppercase tracking-wider rounded-full px-1.5 py-0.5 ${earned ? style.badge : "border border-slate-700 text-slate-600 bg-slate-800/50"}`}
                                    >
                                      {m.tier}
                                    </span>
                                  </div>
                                  <p
                                    className="text-[10px] text-slate-500 leading-snug mt-0.5 truncate"
                                    title={m.description}
                                  >
                                    {m.description}
                                  </p>
                                  ...
                                  <div className="flex items-center justify-between mt-1.5 pt-1.5 border-t border-slate-800/50">
                                    <div className="flex items-center gap-1 text-[9px] font-bold">
                                      <Zap
                                        className={`h-2.5 w-2.5 ${earned ? "text-amber-400" : "text-slate-600"}`}
                                      />
                                      <span
                                        className={
                                          earned
                                ...
                                {earned && (
                                  <CheckCircle2 className="absolute -top-1.5 -right-1.5 h-4 w-4 text-emerald-500 bg-[#090b14] rounded-full shadow-sm" />
                                )}
                              </motion.div>
                            );
                          })}
        ...
        {tab === "achievements" &&
          (() => {
            ...
            const forumRep = Math.max(
              ...
              posts.reduce(
                (s, p) =>
                  s + 10 + p.upvotes_count * 5 + p.comments_count * 1 - 0 * 2,
                0,
              ),
            );
            const FORUM_TIERS = [
            ...
            const tier =
              FORUM_TIERS.slice()
                ...
                .find((t) => forumRep >= t.min) ?? FORUM_TIERS[0];
            const next =
              FORUM_TIERS.find((t) => forumRep < t.max) ??
              FORUM_TIERS[FORUM_TIERS.length - 1];
            const progress =
              next.max === Infinity
                ? 100
                : Math.min(
                  ((forumRep - tier.min) / (next.max - tier.min)) * 100,
                  100,
                );
            const FORUM_MS = [
              {
                id: "fm1",
                title: "First Steps",
                desc: "Post your first discussion",
                xp: 15,
              ...
              },
              {
                id: "fm3",
                title: "Crowd Pleaser",
                desc: "Earn 10+ upvotes across posts",
                xp: 30,
                earned: posts.reduce((s, p) => s + p.upvotes_count, 0) >= 10,
              },
              {
                id: "fm4",
                title: "Forum Legend",
                desc: "Earn 50+ upvotes across posts",
                xp: 100,
                earned: posts.reduce((s, p) => s + p.upvotes_count, 0) >= 50,
              },
              {
                id: "fm5",
                title: "Veteran",
                desc: "Reach Veteran tier (300+ rep)",
                xp: 60,
                earned: forumRep >= 300,
              },
              {
                id: "fm6",
                title: "Scholar Tier",
                desc: "Reach Scholar tier (1500+ rep)",
                xp: 200,
                earned: forumRep >= 1500,
              },
            ];
            ...
            return (
              ...
              >
                ...
                {/* Tier card */}
                <div
                  className={`flex items-center gap-4 rounded-xl border p-4 ${tier.ring} ${tier.bg}`}
                >
                  <div>
                    <p className="text-2xl font-black text-slate-100 tabular-nums">
                      {forumRep.toLocaleString()}
                    </p>
                    <p className="text-[10px] text-slate-500">reputation pts</p>
                  </div>
                  <div className="flex-1">
                    <span
                      className={`rounded-full border px-2.5 py-1 text-xs font-bold ${tier.ring} ${tier.color} bg-slate-900/60`}
                    >
                      {tier.label}
                    </span>
                    {next.max !== Infinity && (
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    {FORUM_MS.map((m, i) => (
                      <motion.div
                        ...
                        className={`relative flex items-start gap-3 rounded-xl border p-3 transition ${m.earned
                          ? "border-indigo-500/30 bg-indigo-500/10"
                          : "border-slate-800 bg-slate-800/40 opacity-40"
                          }`}
                      >
                        <div
                          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border text-[10px] font-black ${m.earned
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-xs font-semibold leading-tight ${m.earned ? "text-slate-100" : "text-slate-600"}`}
                          >
                            {m.title}
                          </p>
                        ...
                        {m.earned && (
                          <CheckCircle2 className="absolute -top-1.5 -right-1.5 h-4 w-4 text-emerald-400 bg-slate-900 rounded-full" />
                        )}
                      </motion.div>
                    ))}
                  </div>
        ...
        {tab === "achievements" &&
          (() => {
            ...
            const totalDl =
              profileData?.libraryNotes.reduce(
                (s, n) => s + (n.downloads_count ?? 0),
                0,
              ) ?? 0;
            ...
            const LIB_MS = [
              ...
              },
              {
                id: "pop_start",
                title: "Popular Start",
                desc: "Your notes get 50 total downloads",
                xp: 20,
                tier: "Bronze",
                earned: totalDl >= 50,
              },
              {
                id: "rising_star",
                title: "Rising Star",
                desc: "200 total downloads on your notes",
                xp: 60,
                tier: "Silver",
                earned: totalDl >= 200,
              },
              {
                ...
                desc: "500 total downloads on your notes",
                xp: 150,
                tier: "Gold",
                earned: totalDl >= 500,
              },
              {
                id: "cons_sharer",
                title: "Consistent Sharer",
                desc: "Upload notes in 3 different subjects",
                xp: 30,
                tier: "Bronze",
              ...
              },
              {
                id: "domain_exp",
                title: "Domain Expert",
                desc: "Upload notes in 5 different subjects",
                xp: 70,
                tier: "Silver",
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {LIB_MS.map((m, i) => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: i * 0.03 }}
                      className={`relative flex items-start gap-3 rounded-xl border p-3 shadow-sm transition ${m.earned
                        ? TIER_C[m.tier]
                        : "border-slate-800 bg-slate-800/40 opacity-40"
                        }`}
                    >
              ...
              {(() => {
                ...
                if (events.length === 0)
                  return (
                    <div className="flex flex-col items-center gap-2 py-10 text-center">
                      ...
                      <p className="text-sm text-slate-400">
                        No activity yet. Start contributing!
                      </p>
                    </div>
                  );
                ...
                return (
                  <div className="relative flex flex-col gap-0">
                    ...
                    {events.map(
                      (
                        { label, sublabel, time, icon: Icon, color, href },
                        i,
                      ) => (
                        <motion.div
                          key={i}
                        ...
                        >
                          ...
                          <div className="flex-1 pt-0.5">
                            {href ? (
                              <Link
                                href={href}
                                className="block text-sm text-slate-300 hover:text-indigo-400 transition-colors"
                              >
                                {label}
                              </Link>
                            ) : (
                      ...
                      {followers.map((entry, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="flex items-center gap-1 text-sm font-medium text-slate-200 truncate">
                                {name}
                          ...
                          .map((entry, i) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="flex items-center gap-1 text-sm font-medium text-amber-100 truncate">
                                    {name}
                                    {entry.profile.is_verified && <BadgeCheck className="h-3.5 w-3.5 shrink-0 text-blue-400" />}
                                  </p>
                          ...
                          .map((entry, i) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="flex items-center gap-1 text-sm font-medium text-slate-300 truncate">
                                    {name}
                                    {entry.profile.is_verified && <BadgeCheck className="h-3.5 w-3.5 shrink-0 text-blue-400" />}
                                  </p>
                                  <p className="text-xs text-slate-500">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <Link
                            ...
                            className="flex items-center gap-1 truncate text-sm font-medium text-slate-200 hover:text-indigo-400 transition"
                          >
                            {name}
                            {entry.profile.is_verified && <BadgeCheck className="h-3.5 w-3.5 shrink-0 text-blue-400" />}
                          </Link>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              await unfollowUser(user.id, entry.userId);
                              setFollowing((prev) =>
                                prev.filter((f) => f.userId !== entry.userId),
                              );
                              setFollowCounts((c) => ({
                                ...c,
                                following: Math.max(0, c.following - 1),
            ...
            <div className="flex items-center gap-3 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
              ...
              <div>
                ...
                <p className="text-xs text-amber-400/60">
                  You can delete posts, notes, and groups below. Changes are
                  permanent.
                </p>
              </div>
            </div>
                          ...
                          try {
                            ...
                            setProfileData((d) =>
                              d
                                ? {
                                  ...d,
                                  forumPosts: d.forumPosts.filter(
                                    (x) => x.id !== p.id,
                                  ),
                                }
                                : d,
                          ...
                          try {
                            ...
                            setProfileData((d) =>
                              d
                                ? {
                                  ...d,
                                  libraryNotes: d.libraryNotes.filter(
                                    (x) => x.id !== n.id,
                                  ),
                                }
                                : d,
      ...
      {/* XP Modal */}
      {showXpModal && profileData?.xpData && (
        <XpModal
          currentXp={profileData.xpData.totalXP}
