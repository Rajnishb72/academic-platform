...
const NAV = [
  ...
  { label: "Bookmarks", href: "/library/bookmarks", icon: Bookmark },
  { label: "Ranking", href: "/library/ranking", icon: Trophy },
  { label: "Milestones", href: "/library/milestones", icon: Medal },
  { label: "My Contributions", href: "/library/collections", icon: FolderHeart },
  { label: "Upload", href: "/library/upload", icon: UploadCloud },
  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)] flex-col" style={{ background: "var(--ax-surface-0)" }}>
      ...
      <div className="relative overflow-hidden border-b" style={{ borderColor: "var(--ax-border)" }}>
        {/* Gradient background */}
        <div className="absolute inset-0 bg-gradient-to-r from-amber-600/10 via-orange-600/5 to-transparent" />
        ...
        <div className="relative mx-auto flex max-w-7xl items-center gap-4 px-6 py-5">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "var(--ax-text-muted)" }}>
              Share, discover & rate study materials
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="sticky top-16 z-20 border-b" style={{ borderColor: "var(--ax-border)", background: "rgba(5, 8, 22, 0.85)", backdropFilter: "blur(16px)" }}>
        <div className="mx-auto max-w-7xl px-6">
          <nav className="flex gap-1 overflow-x-auto py-2 scrollbar-hide" aria-label="Library navigation">
            {NAV.map(({ label, href, icon: Icon }) => {
              ...
              return (
                ...
                >
                  ...
                  <Icon className={`relative z-10 h-4 w-4 ${isActive ? "text-amber-400" : ""}`}
